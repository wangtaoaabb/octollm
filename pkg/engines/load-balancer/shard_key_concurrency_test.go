...
func TestShardKeyConcurrency_No_ShardKey(t *testing.T) {
	...
	t.Run("no shard keys results in round robin behavior", func(t *testing.T) {
		...
		assert.InDelta(t, float64(cur2Val)/float64(cur1Val), 2.0, 0.05, "expected concurrency ratio to reflect rate limits (2:1)")
		assert.InDelta(t, float64(cur3Val)/float64(cur1Val), 3.0, 0.05, "expected concurrency ratio to reflect rate limits (3:1)")
	...
	t.Run("two lbs interleaved", func(t *testing.T) {
		...
		require.True(t, cur1Val > 0, "expected some concurrency on svc1")
		assert.InDelta(t, float64(cur2Val)/float64(cur1Val), 2.0, 0.1, "expected concurrency ratio to reflect rate limits (2:1)")
		assert.InDelta(t, float64(cur3Val)/float64(cur1Val), 3.0, 0.1, "expected concurrency ratio to reflect rate limits (3:1)")
